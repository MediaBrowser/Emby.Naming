...
namespace MediaBrowser.Naming.Tests.Video
{
    ...
    public class MultiPartTests : BaseVideoTest
    {
        ...
        {
            Assert.IsFalse(IsMultiPartFile(@"blah blah"));
            Assert.IsFalse(IsMultiPartFile(@"d:\\music\weezer\\03 Pinkerton"));
            Assert.IsFalse(IsMultiPartFile(@"d:\\music\\michael jackson\\Bad (2012 Remaster)"));

            Assert.IsTrue(IsMultiPartFile(@"blah blah - cd1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - disc1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - disk1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - pt1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - part1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - dvd1"));

            // Add a space
            Assert.IsTrue(IsMultiPartFile(@"blah blah - cd 1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - disc 1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - disk 1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - pt 1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - part 1"));
            Assert.IsTrue(IsMultiPartFile(@"blah blah - dvd 1"));
        ...
        private bool IsMultiPartFile(string path)
        {
            ...
            return parser.Parse(path).IsMultiPart;
        }
    }
}
